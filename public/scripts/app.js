...
function createTweetElement(tweet){
  ...
  `<article class="tweet">
    ...
    <footer>
      <div class="stamp"> ${(tweet.created_at) } </div>
      <section class="tools">
...
$('form').submit(function(events) {
  events.preventDefault(); // This prevent leaving the page for submit
...
function loadTweets() {
  ...
  .then((tweets) => { //change according to what's passed/received in data
    $('#container-tweet').empty()  //empty container so that db doesn't duplicate when form is submitted
    renderTweets(tweets)
  })
}
...
function renderTweets(tweets){
  for (tweet in tweets){
    $('section.container-tweet').append(createTweetElement(tweets[tweet]));
  }
}
